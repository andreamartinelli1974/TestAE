function [d,sec] = portfolio(b,s,f,o,ov,varargin)
%PORTFOLIO Current Bloomberg V3 portfolio data.
%   [D,SEC] = PORTFOLIO(B,S,F) returns the data for the portfolio S for 
%   the fields F.  SEC is the security list that maps the order of the 
%   return data.   The return data, D and SEC, is sorted to match the
%   input order of S.
%
%   [D,SEC] = PORTFOLIO(B,S,F,O,OV) returns the data for the portfolio S for the 
%   fields F using the override fields O with corresponding override
%   values.
%
%   [D,SEC] = PORTFOLIO(B,S,F,O,OV,NAME,VALUE,...) returns the data for the 
%   portfolio S for the fields F using the override fields O with corresponding override
%   values.  NAME/VALUE pairs are used for additional Bloomberg request settings.
%
%   Examples:
%
%   [D,SEC] = PORTFOLIO(C,'US3358-77',...
%       {'PORTFOLIO_MEMBER','PORTFOLIO_MPOSITION','PORTFOLIO_MWEIGHT','PORTFOLIO_DATA'}) 
%   returns the portfolio information for the given portfolio identifier.
%
%   [D,SEC] = PORTFOLIO(C,'US3358-77',...
%       {'PORTFOLIO_MEMBER','PORTFOLIO_MPOSITION','PORTFOLIO_MWEIGHT','PORTFOLIO_DATA'},...
%       {'REFERENCE_DATE'},{'20141103') returns the portfolio information 
%       for the given portfolio identifier for the requested date.
%
