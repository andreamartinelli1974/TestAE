...
%% creates the reference net

hiddenSize = 5;
autoenc1 = trainAutoencoder(X,hiddenSize,...
    'EncoderTransferFunction','logsig', ...
    'DecoderTransferFunction','purelin');
refnet = network(autoenc1);

view(refnet);

refnet.divideFcn = 'dividerand'; % Divide data randomly
refnet.divideMode = 'time'; % 'sample';
refnet.divideParam.trainRatio = 70/100;
refnet.divideParam.valRatio = 15/100;
refnet.divideParam.testRatio = 15/100;
refnet.performFcn = 'mse'; %'sse'; % 'mse' % 'msesparse'; % Loss function used to train the net
refnet.trainFcn = 'trainlm'; % 'trainrp'; % use with mse

refnet = configure(refnet,[X;X2],targets);
[refnet,tr] = train(refnet,[X;X2],targets);

%% creates the net to be checked
...
mynet.layers{1}.size = 5;
mynet.layers{2}.size = N_myFactors;
mynet.biasConnect = true(2,1);

mynet.inputConnect(1,:) = 1;
mynet.outputConnect(end)= 1;
for i = 1:size(mynet.layers)-1
...
mynet.performFcn = 'mse' %'sse'; % 'mse' % 'msesparse'; % Loss function used to train the net
mynet.trainFcn = 'trainlm'; % 'trainrp'; % use with mse
