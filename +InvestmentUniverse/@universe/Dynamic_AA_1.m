...
while t<L
    ...
    %                 AutoEncoder.TrainAE(TrainingSet');
    ...
    if ((QUANT_VIEW_ACTIVE & t_elapsed > min_interval_changes) ... % a Q signal and met condition on min time elapsed
        ...
        if DAA_params.copula_sim == 1 & ~BackTestOnly
            ...
            if DAA_params.UseAutoEncoder == false
                ...
                % the training set has been scaled by the same factor when
                % training has occurred
%                 DataSet4Modeling = (AutoEncoder.EncDecFunction(TrainingSet.*AEparams.multFactor4NumericalStability,'encode')');
            ...
            elseif DAA_params.UseAutoEncoder
                ...
                [~,invariantsNo] = size(X);
                [n1,HiddenSize] = size(X_simulated);
                maxDelay = AutoEncoder.InputParams.Delays(end);
                nsim = DAA_params.ProjectionResampling_numsim;
                X_Resampled = zeros(nsim,invariantsNo,HorizonDays);
                X_Projected = zeros(nsim,invariantsNo);
                path = HorizonDays + maxDelay;
                
                extracted_occurrences = unidrnd(n1,nsim*path,HiddenSize);
                for count_t=1:path
        ...
        if DAA_params.UseAutoEncoder == true
            ...
            U.Debug.AE.WITH.parfortime{counterAE} = parfortime;
