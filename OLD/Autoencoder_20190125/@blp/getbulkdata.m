function [d,sec] = getbulkdata(b,s,f,o,ov,varargin)
%GETBULKDATA Current Bloomberg V3 data.
%   [D,SEC] = GETBULKDATA(B,S,F) returns the data for the fields F for the 
...
%   input order of S.  For bulk data fields, the column names are
%   returned as headers in the output.
%
%   [D,SEC] = GETBULKDATA(B,S,F,O,OV) returns the data for the fields F for the 
%   security list S using the override fields O with corresponding override
%   values.
%
%   [D,SEC] = GETBULKDATA(B,S,F,O,OV,NAME,VALUE,...) returns the data for the fields F for the 
%   security list S using the override fields O with corresponding override
%   values.  NAME/VALUE pairs are used for additional Bloomberg request settings.
%
%   Examples:
%
%   [D,SEC] = GETBULKDATA(b,'ABC US Equity',{'DVD_HIST'}) returns
%   the dividend history for the given security, including the bulk data
%   header fields.
%
      ...
      switch char(field.datatype)
        ...
        case {'SEQUENCE'}
          ...
          for k = 0:numEls - 1;
            ...
            for m = 0:numSubEls-1
              bulkdata{1,m+1} = char(field.getValueAsElement(k).getElement(m).name);
              switch char(field.getValueAsElement(k).getElement(m).datatype)
                case {'DATE'}
                  ...
                  if isempty(tmpdate)
                    bulkdata{k+2,m+1} = NaN;
                  else
                    bulkdata{k+2,m+1} = datenum(tmpdate);
                  end
                case {'FLOAT64','FLOAT32','INT32'}
                  bulkdata{k+2,m+1} = field.getValueAsElement(k).getElement(m).getValueAsFloat64(0);
                otherwise
                  bulkdata{k+2,m+1} = char(field.getValueAsElement(k).getElement(m).getValueAsString(0));
              end
            end
          end
